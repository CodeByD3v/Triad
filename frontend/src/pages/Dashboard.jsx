...
export default function Dashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!issuesRes.ok) throw new Error('Backend error')

      const [issuesData, hotspotsData] = await Promise.all([
        issuesRes.json(),
        hotspotsRes.ok ? hotspotsRes.json() : { hotspots: [] },
      ])
      ...
      {backendDown && (
        <div className="alert-banner error" style={{
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span>⚠️</span>
            <span>Backend is unreachable — showing cached data. Check that your API server is running.</span>
          </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p className="page-subtitle" style={{ marginBottom: 16 }}>
              {issues.length} issues tracked · {resolvedCount} resolved
            </p>
          </div>
            ...
            <MapContainer center={[20.5937, 78.9629]} zoom={5}
              ...
              {hotspots.filter(h => h.risk_level === 'High').map((h, i) => (
                <CircleMarker key={`hs-${i}`}
                  ...
                  radius={30} fillColor="#ef4444" color="#ef4444"
                  weight={2} fillOpacity={0.1} dashArray="6 4">
                  <Popup>
            ...
            <div className="glass-card" style={{
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{
                  ...
                  border: '2px dashed #ef4444', background: 'rgba(239,68,68,0.15)',
                }} /> Predicted Risk
              </span>
